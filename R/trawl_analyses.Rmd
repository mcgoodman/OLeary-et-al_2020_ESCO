...
Trawl surveys were conducted before (2006-2007) and after (2016-2017) the eelgrass decline at the same seven sites in Morro Bay (Figure 1a) during high tide (post-decline: intertidal flats 1.03  ± 0.42  m SD MLLW, channel 1.12 ± 0.22 m SD MLLW; pre-decline: intertidal flats 0.89 m ± 0.33 m SD MLLW, channel 1.02 ± 0.17 m SD MLLW). We surveyed four sites in the back-bay intertidal flats using a beam trawl (2 m wide at the mouth, 4 m long, with a 1 m reinforced cod end and a 4 mm mesh size). The mean depth of the beam trawls was 1.05 ± 0.21 m (SD) in the pre-decline period, and 1.05 ± 0.27 m (SD) in the post-decline period. The remaining three sites surveyed were in the channel running from the mid-bay to the bay mouth. The channel is dredged annually and is too deep for eelgrass to survive, but has persistent eelgrass along the margins. We surveyed the channel sites using an otter trawl (4.6 m wide at the mouth, 7.2 m long with a mesh size of 14 mm in the wings and 8 mm in the code end). The mean depth of channel trawls was 4.68 ± 1.8 m (SD) in the pre-decline period and  5.3 ± 0.95 m (SD) in the post-decline period. The size of the trawls used was based on the equipment used in the early period, and we replicated prior methods for the 2016-2017 surveys. Although trawl size between the two habitats varied, we do not explicitly compare the trawl types, but rather compare results between seasons and period for each respective trawl type. 
...
```{r sample_size}
trawl_info <- read.csv(here("data", "trawl_information.csv"), stringsAsFactors = FALSE)
...
Tide level estimates are given based on trawl start time, using information from Port San Luis, California. Start time was not recorded for some pre-decline surveys. Tidal height is given as meters above the mean lower low water (MLLW).

### By Period, Trawl Type, and Season

```{r tides}
trawl_tides <- trawl_info %>% 
  filter(nchar(time_start) > 0 & Season != "Spring") %>%
  mutate(time_start = as.POSIXct(time_start, format = "%Y-%m-%d %H:%M:%S"))

tide_data <- data.frame(Station = "Port San Luis, Pacific Ocean, California",
                        DateTime = trawl_tides$time_start,
...
trawl_tides %>% 
  filter(Site != "Beam 3") %>% 
  group_by(Period, `trawl type` = TrawlType, Season) %>% 
   summarize(`Mean tidal height (m)` = round(mean(tide), 2), 
             `tidal height SD` = round(sd(tide), 2)) %>% 
  knitr::kable()
```

### By Period and Trawl Type

```{r}
trawl_tides %>% 
  filter(Site != "Beam 3") %>% 
  group_by(Period, `trawl type` = TrawlType) %>% 
   summarize(`Mean tidal height (m)` = round(mean(tide), 2), 
             `tidal height SD` = round(sd(tide), 2)) %>% 
  knitr::kable()
```
...
About 26% of pre-decline surveys lack depth information. 

```{r depth}
trawl_tides %>% 
  filter(!is.na(depth_m) & Site != "Beam 3") %>% 
  group_by(Period, `trawl type` = TrawlType, Season) %>% 
  summarize(`Mean Depth (m)` = round(mean(depth_m), 2),
            `Depth SD` = round(sd(depth_m), 2)) %>% 
  knitr::kable()
```
...
filter_trawls <- function(data) {
  ...
  mutate(Period = ifelse(grepl("200", Year), "Pre-Decline", "Post-Decline") %>% 
           factor(levels = c("Pre-Decline", "Post-Decline"),
                  ordered = TRUE), 
         Site = dplyr::recode(Site, "Otter 1" = "Otter 3", "Otter 3" = "Otter 1"))
}

abundance <- 
  read.csv(here("data", "trawl_fish_counts.csv"), stringsAsFactors = FALSE) %>% 
  filter_trawls %>% 
...
```{r biomass_plot}
biomass <- 
  read.csv(here("data", "trawl_fish_lengths.csv"), stringsAsFactors = FALSE) %>% 
  filter_trawls() %>% 
  ...
  right_join(
    abundance %>%
      select(Period, Season, Year, TrawlType, Site, Replicate)
  ) %>% 
...
bio_count_plot_data <- read_csv(here("data", "trawl_biomass_count_summary.csv"))

panel_labels <- data.frame(
  x = -Inf, y = Inf, 
  TrawlType = fct_rev(factor(rep(c("Channel", "Intertidal Flats"), 2))), 
  type = rep(c("Total Biomass\n(g/Trawl)", "Total Abundance\n(count/Trawl)"), 
             each = 2), 
  label = c("d", "c", "b", "a")
)

bio_count_plot_data %>% 
  mutate(TrawlType = fct_rev(as.factor(TrawlType)),
         type = ifelse(type == "biomass", "Total Biomass\n(g/Trawl)", 
                       "Total Abundance\n(count/Trawl)"), 
  ...
  geom_errorbar(aes(x = Period, ymin = lower, ymax = upper), 
                position = position_dodge(width = 0.9), 
                width = 0.4, size = 1) + 
  geom_line(aes(x = as.numeric(as.factor(Period)), linetype = signif),  
            position = position_dodge(width = 0.9), show.legend = TRUE,
            size = 1.1) + 
  geom_point(aes(shape = Season, size = Season), 
             position = position_dodge(width = 0.9), stroke = 1.2,
             fill = "white") + 
  lemon::facet_rep_grid(type ~ TrawlType, scales = "free_y", switch = "y") + 
  ...
  guides(color = guide_legend(nrow = 1), 
         linetype = guide_legend("adjusted p-value", nrow = 1, 
  ...
  labs(fill = "Season") + 
  expand_limits(y = 0) + 
  geom_text(aes(x = x, y = y, label = label), data = panel_labels, 
...
```{r echo = FALSE}
ggsave(here("figures", "biomass_count_plot.tiff"), width = 20, height = 18, 
...
otter_nmds <- metaMDS(otter_matrix[-18,], distance = "bray", k = 2, trymax = 1000)
...
nmds_plot_data %>% 
  mutate(
    ...
    group = factor(
      ...
      ordered = TRUE), 
    MDS1 = ifelse(TrawlType == "Channel", -MDS1, MDS1),
  ) %>% 
...
otter_permanova_pairs <- pairwise_permanova(
  sp_matrix = otter_matrix, 
  group_var = otter_species$group
)

knitr::kable(otter_permanova_pairs$contrasts)
```

#### SIMPER

For each significant pairwise comparison of interest, we fit a similarity percentages (SIMPER) analysis, reporting abundance for species identified by the SIMPER as contributing to observed differences in community composition more than would be expected at random. For each SIMPER output, relevant species are listed, and an abundance plot is provided. 
...
## Fit pairwise PERMANOVA models
beam_permanova_pairs <- pairwise_permanova(
  sp_matrix = beam_matrix, 
  group_var = beam_species$group
)

knitr::kable(beam_permanova_pairs$contrasts)
```
...
trophic_data <-
  read.csv(here("data", "trawl_fishbase_data.csv"), stringsAsFactors = FALSE) %>% 
...
```{r richness_plot}
## Function to Calculate Shannon index for species abundance vector
...
## Summarize richness and shannon per sample
diversity <-
  read.csv(here("data", "trawl_fish_counts.csv"), stringsAsFactors = FALSE) %>% 
  filter_trawls() %>% 
  mutate(species = paste(Genus, Species)) %>% 
  group_by(TrawlType, Period, Year, Season, Site, Replicate) %>% 
  summarize(richness = sum(Count > 0), 
            shannon = shannon_H(Count))

## Cluster-bootstrap summary of species richness
richness_summary <- diversity %>% 
  group_by(TrawlType, Period, Season) %>% 
  cluster_bootstrap(richness, Site) %>% ungroup() %>% 
  mutate(TrawlType = dplyr::recode(TrawlType, 
                              "Beam" = "Intertidal Flats", 
                              "Otter" = "Channel"))

diversity %>% 
  group_by(TrawlType, Period, Season) %>% 
  summarize(mean = mean(richness), se = sd(richness)/sqrt(n())) %>% 
  mutate(TrawlType = dplyr::recode(TrawlType, 
                              "Beam" = "Intertidal Flats", 
                              "Otter" = "Channel")) %>% 
  ggplot(aes(Period, mean, color = Season)) +
...
```{r richness_lm_otter}
richness_lm_otter <- lm(
  richness ~ Period*Season + Site, 
  data = diversity[diversity$TrawlType == "Otter",]
)

richness_lm_otter %>% anova() %>% 
  rownames_to_column("term") %>% 
  mutate_if(is.numeric, round, digits = 3) %>% 
  knitr::kable()
```
...
richness_lm_beam <- lm(
  richness ~ Period*Season + Site, 
  data = diversity[diversity$TrawlType == "Beam",]
)

richness_lm_beam %>% anova() %>% 
  rownames_to_column("term") %>% 
  mutate_if(is.numeric, round, digits = 3) %>% 
  knitr::kable()
```
...
trawl_counts_nested <- trawl_counts %>% 
  select(-Year, -Year, -Replicate, -Site) %>% 
  group_by(TrawlType, Period, Season) %>% 
  nest_by(.key = "sp_matrix") %>% 
  rowwise() %>% 
  mutate(Chao_data = list(as.abucount(t(sp_matrix))), 
         richness = list(ChaoRichness(Chao_data, datatype = "abundance")), 
         shannon = list(ChaoShannon(Chao_data, datatype = "abundance")))

richness_assymp <- trawl_counts_nested %>% 
  dplyr::select(TrawlType, Period, Season, richness) %>% 
  unnest(cols = c(richness)) %>% 
  mutate(TrawlType = dplyr::recode(TrawlType, "Beam" = "Intertidal Flats", 
                                       "Otter" = "Channels"))
...
richness_assymp %>% 
  ...
  geom_point(position = position_dodge(width = 0.9), size = 3) + 
  geom_errorbar(aes(ymin = `95% Lower`, ymax = `95% Upper`), width = 0.3,
                position = position_dodge(width = 0.9),
                size = 1) +
...
```{r richness_assymp_table}
knitr::kable(richness_assymp)
```
...
```{r shannon_plot}
shannon_summary <- diversity %>%
  group_by(TrawlType, Period, Season) %>% 
  cluster_bootstrap(shannon, Site) %>% ungroup() %>% 
  mutate(TrawlType = dplyr::recode(TrawlType, 
                              "Beam" = "Intertidal Flats", 
                              "Otter" = "Channel"))
...
```{r shannon_lm_otter}
shannon_lm_otter <- lm(
  shannon ~ Period*Season + Site, 
  data = diversity[diversity$TrawlType == "Otter",]
)

shannon_lm_otter %>% anova() %>% 
  rownames_to_column("term") %>% 
  mutate_if(is.numeric, round, digits = 3) %>% 
  knitr::kable()
```
...
shannon_lm_beam %>% anova() %>% 
  rownames_to_column("term") %>% 
  mutate_if(is.numeric, round, digits = 3) %>% 
  knitr::kable()
```

## Assymptotic Diversity

### Plot

```{r shannon_assymp}
shannon_assymp <- trawl_counts_nested %>% 
  dplyr::select(TrawlType, Period, Season, shannon) %>% 
...
```{r shannon_assymp_table}
knitr::kable(shannon_assymp)
```


## Plot: Richness & Diversity

```{r richness_diversity_plot, fig.height = 8, echo = FALSE}
diversity_summary <- richness_summary %>% 
  mutate(type = "species richness") %>% 
  full_join(mutate(shannon_summary, type = "Shannon diversity"))

panel_labels <- data.frame(
  x = -Inf, y = Inf, 
  TrawlType = fct_rev(factor(rep(c("Channel", "Intertidal Flats"), 2))), 
  type = rep(c("species richness", "Shannon diversity"), each = 2), 
  label = c("d", "c", "b", "a")
)
...
```{r include = FALSE}
ggsave(here("figures", "richness_diversity_plot.tiff"), width = 20, height = 18, 
       units = "in", scale = 0.5, dpi = 500)
```